...
document
  ...
  .addEventListener('click', connectWebsites);

function connectWebsites() {
  fetch('https://mayur-bankist-transaction.netlify.app/')
    .then(response => {
      // Process the response from the other website
      if (response.ok) {
...
const bankURL = document.querySelector('.bankistUrl');
const newBank = bankURL.addEventListener('click', function (e) {
  e.preventDefault();
  window.location.href = 'https://mayur-bankist-transaction.netlify.app/';
});
newBank();

// One way of passing values in the given function
// nav.addEventListener('mouseover', function (e, opacity) {
//   handleHover(e, 0.6);
// });
